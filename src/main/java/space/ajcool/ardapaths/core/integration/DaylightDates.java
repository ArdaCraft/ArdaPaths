...
public final class DaylightDates {
    ...
    /**
     * Converts a provider calendar date and wall-clock time into DaylightChangerStruggle ticks.
     *
     * @param date   provider calendar date
     * @param hour   wall-clock hour
     * @param minute wall-clock minute
     * @return provider ticks that display as the supplied date and time
     */
    static long toProviderTicks(LocalDate date, int hour, int minute) {
        long minutesFromDawn = (hour * 60L) + minute - (6L * 60L);
        long timeTicks = Math.round(minutesFromDawn * TimeOfDay.DAY_TICKS / 1440.0D);
        return (providerDayIndex(date) * TimeOfDay.DAY_TICKS) + timeTicks;
    }
